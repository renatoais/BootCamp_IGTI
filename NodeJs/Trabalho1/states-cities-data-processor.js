...
const getStatesWithTotalCities = () => {
  return StatesCitiesFileProcessor.getAllStates().map((state) => ({
    uf: state.Sigla,
...
const getCityWithBiggestName = (uf) => {
  return StatesCitiesFileProcessor.getStateCities(uf)
...
const printBiggestNameCities = () => {
  const data = StatesCitiesFileProcessor.getAllStates()
    .map((state) => ({
      uf: state.Sigla,
